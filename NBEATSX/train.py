...
if __name__ == '__main__':
    ...
    for v in range(1):
        ...
        forecast_length = 1
        backcast_length = 7
        # model = NBeatsNet(device=torch.device('cuda'),stack_types=(NBeatsNet.TREND_BLOCK, NBeatsNet.SEASONALITY_BLOCK),forecast_length=forecast_length,backcast_length=backcast_length,extra_dims = 6,hidden_layer_units=64)
        model = NBeatsNet(device=torch.device('cuda'),stack_types=(NBeatsNet.GENERIC_BLOCK, NBeatsNet.GENERIC_BLOCK),forecast_length=forecast_length,backcast_length=backcast_length,extra_dims = 6,hidden_layer_units=64)
        ...
        for i in range(epochs):
            ...
            for (data, label) in train_loader:
                ...
                train_data_back = train_data[:,:,1]
                train_data = train_data.reshape(train_data.shape[0],-1)
                train_label = label.reshape(-1).to(device)
                
                
                _,y_pred = model(train_data_back,train_data)
            ...
            for (data, label) in val_loader:
                ...
                with torch.no_grad():
                    ...
                    _,val_pred = model(val_data_back,val_data)
